...
for tradeDay in range(startDay, endDay + 1):
    ...
    for element in range(2, 6):
        for hour in range(1, 25):
            new = soup.select(f"#footable_kontrakty_godzinowe > tbody > tr:nth-child({hour}) > td:nth-child({element})")[0]
    ...
    df.to_excel(f'C:/Users/waliszewski_a/Documents/pyData/tge/{dateDelivery}_tge.xlsx')